...
fpm_sampling = 0.05*lambda_m/D*180/np.pi*3600
...
def run_model(npix=1000,oversample=2):
    reload(poppy)
    ...
    fpm_plane = poppy.CircularAperture(radius=diam_oap5/2,name="FPM (None)") # just using a placeholder diameter for the empty FPM plane
    spc1.add_optic(fpm_plane, distance=d_oap5_fpm)

    spc1.describe()
    
    lambda_m = 0.73e-6*u.m
    plt.figure(figsize=(15,15))
    psf,wfs = spc1.calc_psf(wavelength=lambda_m, display_intermediates=True, return_intermediates=True)
    plt.subplots_adjust(hspace=0.25, wspace=0.25)

    wfnum = -1
    wf_int = wfs[wfnum].intensity
    wf_amp = wfs[wfnum].amplitude
    wf_phs = wfs[wfnum].phase
    pxscl = wfs[wfnum].pixelscale.value; print(wfs[wfnum].pixelscale)
    n = wf_int.shape[0]
    ext = pxscl * n/2
    ...
    # wf = misc.pol2rect(wf_amp,wf_phs)
    wf = wfs[wfnum].wavefront
    wf = misc.ffts(wf,1)
    fpm = FPM.amplitude; print(fpm.shape,fpm)
    nfpm = fpm.shape[1]
    fpm_sampling_lam = fpm_sampling * fpm_sampling_lambda_m / lambda_m # normalize the fpm_sampling based on wavelength
    wf = misc.mft2(wf, fpm_sampling_lam, pupil_diam_pix, nfpm, -1)   # MFT to highly-sampled focal plane
    wf *= fpm
    fpm = 0 # reset the fpm variable for memory purposes
    wf = misc.mft2(wf, fpm_sampling_lam, pupil_diam_pix, n, +1)  # MFT to virtual pupil 
    wf = misc.ffts( wf, -1 )    # back to normally-sampled focal plane

    amp,phs = misc.rect2pol(wf)
    fig,ax = plt.subplots(nrows=1,ncols=2,figsize=(10,4),dpi=150)
    im1=ax[0].imshow(np.abs(amp)**2, norm=LogNorm(), cmap='magma')
    fig.colorbar(im1,ax=ax[0])
    im2=ax[1].imshow(phs, cmap='magma')
    fig.colorbar(im2,ax=ax[1])

    fpm_amp_fname = 'fpm_wf_amp.fits'
    hdr = fits.Header()
    hdr['PIXELSCL'] = pxscl

    wf_hdu = fits.PrimaryHDU(amp, header=hdr)
    wf_hdu.writeto(fpm_amp_fname, overwrite=True)

    fpm_phs_fname = 'fpm_wf_phs.fits'
    hdr = fits.Header()
    hdr['PIXELSCL'] = pxscl

    wf_hdu = fits.PrimaryHDU(phs, header=hdr)
    wf_hdu.writeto(fpm_phs_fname, overwrite=True)

    # test 
    wfin = poppy.FresnelWavefront(0.5*u.m, wavelength=0.73e-6,npix=npix,oversample=2)
    wfin.wavefront = wf
    wfin.pixelscale = wfs[wfnum].pixelscale

    plt.figure(figsize=(8,4)); wfin.display(what='both')
